...
def _write_manifest(
    ...
    *,
    ...
    :return: None
    """
    version_dir = root / "automation-manual-studio" / f"v{version}" / platform if platform else root / "automation-manual-studio" / f"v{version}"
    ...
    payload = {
        ...
        "package": {
            "url": (
                f"automation-manual-studio/v{version}/{platform}/package.zip"
                if platform
                else f"automation-manual-studio/v{version}/package.zip"
            ),
            "size": len(content),
...
def test_service_lists_remote_versions_sorted_by_version(tmp_path: Path) -> None:
    ...
    :return: None
    """
    _write_manifest(tmp_path, version="1.0.5", content=b"old")
    _write_manifest(tmp_path, version="1.0.7", content=b"new")
    _write_manifest(tmp_path, version="1.0.6", content=b"mid")
    service = UpdateService(UpdateToolSettings(nas_root=tmp_path))
...
def test_service_list_remote_versions_prefers_versions_index(tmp_path: Path) -> None:
    """验证存在 versions.json 时按索引列出版本。
        ...
        json.dumps(
            {
                ...
                "versions": [
                    {
                        "version": "1.0.5",
                        "manifest_url": "automation-manual-studio/v1.0.5/latest.json",
                    }
                ],
            }
        ),
...
def test_service_get_remote_manifest_supports_platform_version(tmp_path: Path) -> None:
    ...
    assert manifest.package.url == "automation-manual-studio/v1.0.6/macos/package.zip"
