...
def test_agent_marks_ready_before_applying_and_restarts_bootstrap(tmp_path: Path) -> None:
...
@pytest.mark.parametrize("operation_id", ["../escape", "/tmp/escape", "nested/request", "with space"])
def test_agent_rejects_operation_ids_that_cannot_be_operation_filenames(tmp_path: Path, operation_id: str) -> None:
...
def test_agent_rejects_request_path_outside_its_operation_contract(tmp_path: Path) -> None:
...
def test_agent_switches_installed_release_then_restarts_bootstrap(tmp_path: Path) -> None:
...
def test_agent_failure_marks_status_and_never_starts_bootstrap(tmp_path: Path) -> None:
    ...
    (install_root / "current.json").write_text(
        json.dumps({"version": "1.0.0", "release_dir": "releases/1.0.0", "executable": "Product"}),
        encoding="utf-8",
    )
    request = create_apply_request(
        install_root=install_root,
    ...
    def apply_pending(**kwargs: object) -> RuntimeResult:
        ...
        raise UpdateError(UpdateErrorCode.PACKAGE_HASH_MISMATCH, "package hash mismatch")
    ...
    with pytest.raises(UpdateError, match="PACKAGE_HASH_MISMATCH: package hash mismatch"):
        UpdateAgent(
            apply_pending=apply_pending,
            launch_bootstrap=launch_bootstrap,
...
def test_agent_launcher_waits_for_ready_then_allows_host_handoff(tmp_path: Path) -> None:
    ...
    def popen(args: list[str], cwd: str, close_fds: bool, **kwargs: object):  # noqa: ANN001
        ...
        status_path.write_text(
            json.dumps({"schema_version": 1, "operation_id": "update-003", "phase": "ready"}),
            encoding="utf-8",
        )
    ...
    assert calls == [[str(agent_executable), "apply", "--request", str(started.request_path)]]
    assert json.loads(handoff_path.read_text(encoding="utf-8"))["operation_id"] == "update-003"


def test_agent_launcher_fails_fast_when_agent_exits_before_ready(tmp_path: Path) -> None:
    ...
    launcher = UpdateAgentLauncher(agent_executable, popen=lambda *args, **kwargs: Process(), poll_interval=0.001)

    with pytest.raises(UpdateError, match="exited before ready with code 17"):
        launcher.start(request, ready_timeout=5.0)

    status = read_agent_status(install_root / "operations" / "agent-exited.request.json")
    assert status["phase"] == "failed"


def test_agent_launcher_terminates_waiting_agent_after_ready_timeout(tmp_path: Path) -> None:
    ...
    status = read_agent_status(install_root / "operations" / "agent-timeout.request.json")
    assert process.terminated is True
    assert status["phase"] == "failed"


def test_agent_performs_real_process_handoff_and_bootstrap_starts_new_release(tmp_path: Path) -> None:
    ...
    pending_path.write_text(
        json.dumps({"install_root": str(install_root), "package_path": str(package_path), "manifest": manifest.to_payload()}),
        encoding="utf-8",
    )
    ...
    old_process = subprocess.Popen([sys.executable, "-c", "import time; time.sleep(0.15)"])
    request = create_apply_request(
        install_root=install_root,
        pending_path=pending_path,
