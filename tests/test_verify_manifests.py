...
def test_verify_manifest_accepts_qt_ifw_repository_contract(tmp_path: Path) -> None:
    """验证 manifest 支持 Qt IFW 仓库契约字段。
...
def test_verify_manifest_rejects_invalid_ifw_repository_scheme(tmp_path: Path) -> None:
    """验证 IFW 仓库 URL 不允许不安全协议。
    ...
    payload = {
        ...
        "notes": "release",
        "installer_kind": "qt_ifw",
        "repository_url": "http://example.com/ifw-repository",
        "package": {
            "url": "automation-manual-studio/v1.0.6/package.zip",
            "size": package_path.stat().st_size,
            "sha256": hashlib.sha256(b"release").hexdigest(),
    ...
    assert "repository_url must use https or file scheme" in "\n".join(
