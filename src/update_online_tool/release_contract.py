...
class ReleaseArtifactContract:
    ...
    def from_payload(cls, payload: dict[str, Any]) -> "ReleaseArtifactContract":
        ...
        if not isinstance(required_payload, list):
            raise UpdateError(
                UpdateErrorCode.MANIFEST_INVALID,
                "release contract required_paths must be an array",
            )
        return cls(
            app_id=validate_release_component(
                _require_text(payload.get("app_id"), "release contract app_id"),
                "release contract app_id",
                error_code=UpdateErrorCode.MANIFEST_INVALID,
            ),
            version=_normalize_version(
                _require_text(payload.get("version"), "release contract version")
            ),
            platform=validate_release_platform(
                _require_text(payload.get("platform"), "release contract platform"),
                error_code=UpdateErrorCode.MANIFEST_INVALID,
            ),
            entry_path=_normalize_relative_path(
                _require_text(payload.get("entry_path"), "release contract entry_path")
            ),
            required_paths=_normalize_required_paths(
                required_payload, error_code=UpdateErrorCode.MANIFEST_INVALID
            ),
        )
...
def write_release_contract(
    release_dir: Path, contract: ReleaseArtifactContract
) -> Path:
    """原子写入 release 自描述完整性契约。"""
    root = Path(release_dir)
    if not root.is_dir():
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID, f"release directory not found: {root}"
        )
    ...
    try:
        ...
        if temp.exists():
            temp.unlink()
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID, f"cannot write release contract: {target}"
        ) from exc
    return target
...
def read_release_contract(release_dir: Path) -> ReleaseArtifactContract | None:
    ...
    if not path.is_file():
        raise UpdateError(
            UpdateErrorCode.MANIFEST_INVALID, f"release contract is not a file: {path}"
        )
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except OSError as exc:
        raise UpdateError(
            UpdateErrorCode.MANIFEST_INVALID, f"cannot read release contract: {path}"
        ) from exc
    except json.JSONDecodeError as exc:
        raise UpdateError(
            UpdateErrorCode.MANIFEST_INVALID,
            f"release contract is not valid JSON: {path}",
        ) from exc
    if not isinstance(payload, dict):
        raise UpdateError(
            UpdateErrorCode.MANIFEST_INVALID, "release contract must be a JSON object"
        )
...
def validate_release_artifact(
    *,
    ...
    normalized_version = _normalize_version(
        _require_text(version, "release version"),
        error_code=UpdateErrorCode.SETTINGS_INVALID,
    )
    normalized_entry = _normalize_relative_path(
        _require_text(entry_path, "release entry_path")
    )
    if not root.is_dir():
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID, f"release directory not found: {root}"
    ...
    if not _is_entry_path(root / normalized_entry):
        raise UpdateError(
            ...
            f"release entry not found: {root / normalized_entry}",
        )

    caller_required = _normalize_required_paths(
        required_paths, error_code=UpdateErrorCode.SETTINGS_INVALID
    )
...
def normalize_release_required_paths(value: object) -> tuple[str, ...]:
    ...
    if not isinstance(value, (list, tuple)):
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID, "release_required_paths must be an array"
        )
...
def _validate_contract_identity(
    ...
    *,
    ...
    if contract.entry_path != entry_path:
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID,
            "release contract entry_path does not match active entry",
        )
    if app_id and contract.app_id != app_id.strip():
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID,
            "release contract app_id does not match active application",
        )
    if platform and contract.platform != platform.strip():
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID,
            "release contract platform does not match active platform",
        )
...
def _ensure_required_path(release_dir: Path, relative_path: str) -> None:
    ...
    if not target.exists() or not target.is_file():
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID,
            f"required release path not found: {target}",
        )
    try:
        ...
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID,
            f"required release path escapes release root: {target}",
        ) from exc


def _normalize_required_paths(
    value: object, *, error_code: UpdateErrorCode
) -> tuple[str, ...]:
    """验证并去重必需资源相对路径。"""
    if not isinstance(value, (list, tuple)):
    ...
    for item in value:
        if not isinstance(item, str):
            raise UpdateError(
                error_code, "release required paths must contain non-empty strings"
            )
...
def _normalize_relative_path(
    value: str, *, error_code: UpdateErrorCode = UpdateErrorCode.MANIFEST_INVALID
) -> str:
    """将 POSIX 相对路径规范化为跨平台文件系统路径文本。"""
    return validate_release_relative_path(value, "release path", error_code=error_code)


def _normalize_version(
    value: str,
    *,
    error_code: UpdateErrorCode = UpdateErrorCode.MANIFEST_INVALID,
) -> str:
    """去除可选 v 前缀并保留版本文本。"""
    return normalize_release_version(value, error_code=error_code)
...
def _require_text(value: object, label: str) -> str:
    ...
    if not text:
        raise UpdateError(
            UpdateErrorCode.MANIFEST_INVALID, f"{label} must be a non-empty string"
        )
    return text
...
def _is_entry_path(path: Path) -> bool:
    ...
    return executable_dir.is_dir() and any(
        candidate.is_file() for candidate in executable_dir.iterdir()
    )
