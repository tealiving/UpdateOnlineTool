...
def load_hmac_key(path: Path) -> bytes:
    ...
    try:
        ...
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID, f"signature key cannot be read: {path}"
        ) from exc
    if not text:
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID, "signature key must be non-empty"
        )
    try:
...
def sign_manifest_payload(
    payload: dict[str, Any], *, key: bytes, key_id: str = ""
) -> dict[str, Any]:
...
def sign_manifest_payload_with_key_file(
    ...
    *,
    ...
    if _looks_like_pem(key_path):
        return _sign_ed25519_manifest_payload(
            payload, private_key_path=key_path, key_id=key_id
        )
...
def verify_manifest_signature(payload: dict[str, Any], *, key: bytes) -> None:
    ...
    if not isinstance(signature, dict):
        raise UpdateError(
            UpdateErrorCode.MANIFEST_INVALID, "manifest signature is required"
        )
    ...
    if not hmac.compare_digest(value.strip().lower(), expected):
        raise UpdateError(
            UpdateErrorCode.MANIFEST_INVALID, "manifest signature mismatch"
        )


def verify_manifest_signature_with_key_file(
    payload: dict[str, Any], *, key_path: Path
) -> None:
    ...
    if _looks_like_pem(key_path):
        if algorithm != ED25519_SIGNATURE_ALGORITHM:
            raise UpdateError(
                UpdateErrorCode.MANIFEST_INVALID,
                f"PEM signature key requires {ED25519_SIGNATURE_ALGORITHM}, got: {algorithm}",
            )
    ...
    if algorithm != SIGNATURE_ALGORITHM:
        raise UpdateError(
            UpdateErrorCode.MANIFEST_INVALID,
            f"non-PEM signature key requires {SIGNATURE_ALGORITHM}, got: {algorithm}",
        )
...
def _sign_ed25519_manifest_payload(
    ...
    *,
    ...
    signature_value = b64encode(
        _load_ed25519_private_key(private_key_path).sign(
            canonical_manifest_bytes(unsigned_payload)
        )
    ).decode("ascii")
    signed_payload = dict(unsigned_payload)
    signed_payload["signature"] = {
...
def _verify_ed25519_manifest_signature(
    payload: dict[str, Any], *, public_key_path: Path
) -> None:
    ...
    try:
        ...
        _load_ed25519_public_key(public_key_path).verify(
            signature_bytes, canonical_manifest_bytes(unsigned_payload)
        )
...
def _load_ed25519_private_key(path: Path) -> Ed25519PrivateKey:
    ...
    try:
        ...
        raise UpdateError(
            ...
            f"Ed25519 private key cannot be read: {path}",
        ) from exc
    if not isinstance(key, Ed25519PrivateKey):
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID, "signature private key must be Ed25519"
        )
    return key
...
def _load_ed25519_public_key(path: Path) -> Ed25519PublicKey:
    ...
    try:
        ...
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID,
            f"Ed25519 public key cannot be read: {path}",
        ) from exc
    try:
    ...
    try:
        ...
        raise UpdateError(
            UpdateErrorCode.SETTINGS_INVALID, "signature public key must be Ed25519"
        ) from exc
