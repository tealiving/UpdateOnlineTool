...
def copy_package_with_verification(
    *,
    ...
    if not source_path.is_file():
        raise UpdateError(UpdateErrorCode.PACKAGE_NOT_FOUND, f"package not found: {source_path}")
    ...
    try:
        with source_path.open("rb") as source_file, temp_path.open("wb") as target_file:
            while True:
                if cancellation_token is not None and cancellation_token.cancelled:
                    raise UpdateError(UpdateErrorCode.OPERATION_CANCELLED, "package copy cancelled")
        ...
        return PreparedPackage(package_path=target_path, sha256=actual_sha256, verified=True)
    except Exception:
        if temp_path.exists():
            temp_path.unlink()
        raise
